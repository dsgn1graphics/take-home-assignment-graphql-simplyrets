...
describe('Query', () => {
	...
	describe('[listings]', () => {
		...
		it('returns the expected data collection for all listings', async () => {
			...
			const res = await resolvers.Query.listings(null, { limit: 20 }, mockContext);
	...
	describe('[listing]', () => {
		...
		it('returns the requested listing', async () => {
			...
			const res = await resolvers.Query.listing(null, { mlsId: "1005192" }, mockContext);
	...
	describe('[properties]', () => {
		...
		it('returns a collection of properties for all listings', async () => {
			...
			const res = await resolvers.Query.properties(null, { limi: 5 }, mockContext);
	...
	describe('[property]', () => {
		...
		it('returns the property associated to the provided listing', async () => {
			...
			const res = await resolvers.Query.property(null, { mlsId: "1005192" }, mockContext);
	...
	describe('[location]', () => {
		...
		it('returns a collection of listings by the provided city', async () => {
			...
			const res = await resolvers.Query.listingsByLocation(null, { city: 'Houston' }, mockContext);
